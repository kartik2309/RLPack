...
PYBIND11_MODULE(C_Memory, m) {
  ...
  pybind11::bind_map<std::map<std::string, torch::Tensor>>(m, "C_Memory_TensorMap")
      ...
      .def(pybind11::pickle(
           ...
           pybind11::return_value_policy::reference);
  /*
   * This is the binding of STL std::map<std::string, std::vector<torch::Tensor>> aliased C_Memory_MapOfVectorOfTensors.
   * C_Memory_MapOfVectorOfTensors will be returned from the following functions:
   * - deref_transition_information_map (in C_Memory::C_MemoryData, access available as property `transitions` in Python).
   */
  pybind11::bind_map<std::map<std::string, std::vector<torch::Tensor>>>(m, "C_Memory_MapOfVectorOfTensors")
  ...
  pybind11::bind_map<std::map<std::string, std::vector<int64_t>>>(m, "C_Memory_MapOfVectorOfInt64")
      ...
      .def(pybind11::pickle(
               ...
               [](pybind11::dict &init) {
                 std::map<std::string, std::vector<int64_t>> cMemoryMapOfVectorOfInt64;
                 for (auto &pair : init) {
                   cMemoryMapOfVectorOfInt64[pair.first.cast<std::string>()] = pair.second.cast<std::vector<int64_t>>();
                 }
